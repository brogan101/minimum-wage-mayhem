...
const SCREENSHOT_PATH := "res://artifacts/phase17_rendered_demo.png"
...
func _run():
	...
	if err != OK:
		...
		quit(1)
		return
	print("[PASS] Phase 17 rendered screenshot saved: " + SCREENSHOT_PATH)
	main.queue_free()
	await process_frame
	quit(0)
...
func _add_overview_camera():
	...
	camera.name = "Phase17OverviewCamera"
	camera.fov = 58.0
	root.add_child(camera)
	camera.look_at_from_position(Vector3(6.8, 6.1, 8.4), Vector3(0.0, 0.9, -0.4), Vector3.UP)
	camera.current = true
