...
func _run_check():
	...
	var spawn_xz_delta = Vector2(player.global_position.x, player.global_position.z).distance_to(Vector2(0, 4))
	_assert(spawn_xz_delta < 0.25 and player.global_position.y > 0.5, "Player spawn is stable and predictable")
	...
	_assert(controller != null and float(controller.get("mouse_sensitivity")) <= 0.0018, "Mouse look is calmer for first shift")
	_assert(controller != null and float(controller.get("controller_look_sensitivity")) <= 2.1, "Controller look default is calmer")
	...
	_assert(hud.guidance_label.text.contains("ORDER TICKET") and hud.guidance_label.text.contains("CLOCK OUT"), "First-shift guidance is understandable without docs")
