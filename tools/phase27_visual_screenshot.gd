...
const SCREENSHOT_PATH := "res://artifacts/phase27_visual_asset_prop_texture.png"
...
func _run():
	...
	if err != OK:
		...
		quit(1)
		return
	print("[PASS] Phase 27 rendered screenshot saved: " + SCREENSHOT_PATH)
	main.queue_free()
	await process_frame
	quit(0)
...
func _add_overview_camera():
	...
	camera.name = "Phase27VisualOverviewCamera"
	camera.fov = 60.0
	root.add_child(camera)
	camera.look_at_from_position(Vector3(6.7, 6.2, -6.9), Vector3(0.0, 1.05, -0.1), Vector3.UP)
	camera.current = true
