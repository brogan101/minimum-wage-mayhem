...
const SCREENSHOT_PATH := "res://artifacts/phase21_art_direction_demo.png"
...
func _run():
	...
	if err != OK:
		...
		quit(1)
		return
	print("[PASS] Phase 21 rendered screenshot saved: " + SCREENSHOT_PATH)
	main.queue_free()
	await process_frame
	quit(0)
...
func _add_overview_camera():
	...
	camera.name = "Phase21OverviewCamera"
	camera.fov = 57.0
	root.add_child(camera)
	camera.look_at_from_position(Vector3(6.6, 6.2, 8.2), Vector3(-0.4, 1.0, -0.2), Vector3.UP)
	camera.current = true
